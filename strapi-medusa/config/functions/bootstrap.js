...
async function createMedusaUser(medusaRoleId) {
  ...
  params.password = await strapi.plugins['users-permissions'].services.user.hashPassword({password: params.password});
  const user = await strapi.query('user', 'users-permissions').create(params);
  if (user && user.id) {
    strapi.log.info(`User "medusa_user" created successfully with id ${user.id}`)
    return user.id
  } else {
